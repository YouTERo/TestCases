...
def create_test_item(base_url):
    ...
    status_msg = response.json().get("status", "")    #парс Id
...
class TestPositiveCases:
    ...
    def test_tc1_create_valid_item(self, base_url): #TC-1. Создание объявления с валидными данными
        payload = {
            "sellerID": 452612,
            "name": "iPhone 15",
            "price": 100000,
    ...
    def test_tc2_get_item_by_valid_id(self, create_test_item, base_url): #TC-2. Получение существующего объявления по корректному id
    ...
    def test_tc3_get_items_by_seller_id(self, create_test_item, base_url): #TC-3. Получение списка всех объявлений для продавца по id
    ...
    def test_tc4_get_statistic_by_item_id(self, create_test_item, base_url): #TC-4. Получение статистики по существующему item_id
        item_id = create_test_item["id"]
    ...
    def test_tc5_create_item_boundary_values(self, base_url): #TC-5. Создание объявления с граничными значениями (Ожидаем отказ API)
        payload = {
            "sellerID": 452612,
            "name": "a",
            "price": 0,  
...
class TestNegativeCases:
    ...
    def test_tc6_create_item_missing_required_field(self, base_url): #TC-6. Создание объявления с пропуском обязательного поля (name)
        payload = {
            "sellerID": 452612,
            "price": 100000,
            "statistics": {"likes": 10, "viewCount": 100, "contacts": 3}
    ...
    def test_tc7_create_item_invalid_data_type(self, base_url): #TC-7. Создание объявления с некорректным типом данных
        payload = {
            "sellerID": 452612,
            "name": "iPhone 15",
            "price": "free",
    ...
    def test_tc8_get_item_nonexistent_id(self, base_url): #TC-8. Запрос несуществующего id объявления (валидный UUID)
        fake_uuid = str(uuid.uuid4())
        response = requests.get(f"{base_url}/api/1/item/{fake_uuid}")
        assert response.status_code == 404

    def test_tc9_get_item_invalid_id_format(self, base_url): #TC-9. Запрос с некорректным форматом id объявления
        response = requests.get(f"{base_url}/api/1/item/123-abc")
        assert response.status_code == 400

    def test_tc10_get_items_nonexistent_seller(self, base_url): #TC-10. Запрос объявлений несуществующего продавца
        response = requests.get(f"{base_url}/api/1/99999999999/item")
        assert response.status_code == 200
        assert response.json() == []

    def test_tc11_get_items_invalid_seller_id(self, base_url): #TC-11. Запрос некорректного sellerID (строка)
...
class TestCornerCases:
    ...
    def test_tc12_create_identical_items(self, base_url): #TC-12. Создание двух объявлений подряд с абсолютно идентичным телом
        payload = {
            "sellerID": 111222,
            "name": "Clone Item",
            "price": 500,
    ...
    def test_tc13_idempotency_get_item(self, create_test_item, base_url): #TC-13. Выполнение 5 последовательных GET запросов к одному и тому же id
        item_id = create_test_item["id"]
    ...
    def test_tc14_create_item_extreme_price(self, base_url): #TC-14. Создание объявления с экстремально большой ценой
        payload = {
            "sellerID": 452612,
            "name": "Expensive Item",
            "price": 2 ** 63 - 1,
    ...
    def test_tc15_delete_and_get_item(self, create_test_item, base_url): #TC-15. Удаление объявления и попытка получить его снова
        item_id = create_test_item["id"]
    ...
    def test_tc16_repeated_delete(self, create_test_item, base_url): #TC-16. Повторное удаление
        item_id = create_test_item["id"]
...
class TestNonFunctionalCases:
    ...
    def test_tc17_get_item_response_time(self, create_test_item, base_url): #TC-17. Проверка времени ответа на запрос GET
        item_id = create_test_item["id"]
        response = requests.get(f"{base_url}/api/1/item/{item_id}")

        assert response.elapsed.total_seconds() < 1.5

    def test_tc18_1_post_without_content_type(self, base_url): #TC-18 (1). Попытка выполнить POST запрос без Content-Type
    ...
    def test_tc18_2_save_with_emoji(self, base_url): #TC-18 (2). Сохранение с эмодзи в name
        payload = {
            "sellerID": 452612,
            "name": "MacBook Pro 💻✨",
            "price": 250000,
...
class TestE2EScenarios:
    ...
    def test_e2e_full_item_lifecycle(self, base_url):
        ...
        post_resp = requests.post(f"{base_url}/api/1/item", json=payload) #Создание объявления
        assert post_resp.status_code == 200, "Не удалось создать объявление"

        status_msg = post_resp.json().get("status", "") # Парсим ID из ответа API
        item_id = status_msg.split(" - ")[-1]
        assert len(item_id) > 10, "Некорректный ID в ответе"

        get_resp = requests.get(f"{base_url}/api/1/item/{item_id}") #Просмотр
        ...
        seller_resp = requests.get(f"{base_url}/api/1/{seller_id}/item") #Список продавца
        ...
        stat_resp = requests.get(f"{base_url}/api/1/statistic/{item_id}") #Статистика объявления
        ...
        delete_resp = requests.delete(f"{base_url}/api/2/item/{item_id}") #Удаление объявления
        assert delete_resp.status_code == 200, "Не удалось удалить объявление"

        get_deleted_resp = requests.get(f"{base_url}/api/1/item/{item_id}") #Проверка удаления
    ...
    def test_e2e_multiple_items_for_one_seller(self, base_url):
        ...
        for item in items_to_create: # Создание 3 объявлений
            payload = {
        ...
        seller_resp = requests.get(f"{base_url}/api/1/{unique_seller_id}/item") #Проверка списка объявлений у продавца
        assert seller_resp.status_code == 200
